                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                      {stories.length > 0 ? (
                        stories.map((item) => (
                          <Card
                            key={item.id}
                            className={`overflow-hidden w-full border transition-transform hover:scale-[1.02] cursor-pointer ${form.watch('storyId') === item.id ? 'border-primary border-2' : 'border-gray-200 dark:border-gray-700'}`}
                            onClick={() => {
